...
def get_timer_overhead(repeats, warmup):
    time_list = []
...
if __name__ == "__main__":
    ...
    timer_overhead = get_timer_overhead(TIMER_REPEATS, TIMER_WARMUP)
    ...
    time_createNewComponent = end_time - start_time - timer_overhead

    #time hwloc_parser
    start_time = time.perf_counter()
    ret = syge.parseHwlocOutput(n, xmlPath)
    end_time = time.perf_counter()
    time_parseHwlocOutput = end_time - start_time - timer_overhead
    if ret != 0:
        print("failed parsing hwloc")
        sys.exit(1)

    #time get a vector with all Components (of hwloc parser)
    start_time = time.perf_counter()
    hwlocComponentList = n.GetComponentsInSubtree()
    end_time = time.perf_counter()
    time_GetHwlocSubtreeNodeList = end_time - start_time - timer_overhead

    #time caps-numa-benchmark parser
    start_time = time.perf_counter()
    ret = syge.parseCapsNumaBenchmark(n, bwPath, ";")
    end_time = time.perf_counter()
    time_parseCapsNumaBenchmark = end_time - start_time - timer_overhead
