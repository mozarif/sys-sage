...
| macros for type registry |
| ------------------------ |
| SYS_SAGE_REGISTER_TYPE_TRAIT |
| SYS_SAGE_REGISTER_TEMPLATED_TYPE_TRAIT |

Both generate some meta information that is used to register a type **implicitly**.
...
Furthermore, consider the following example: Assume the file `topo.json` has the following content
...
which was generated by the this code:
...
Another important thing to note is that we recommend putting the macro for registering a (templated) type in the file in which the (templated) type is defined. e.g. the header file.
If this is not possible, e.g. you want to register a type from a third-party library, then create a new header file that contains all registration macros and include that header whenever you
